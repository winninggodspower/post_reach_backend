...
urlpatterns = [
    ...
    path(
        "youtube/auth-url/",
        YoutubeAuthViewSet.as_view({"get": "auth_url"}),
        name="youtube-auth-url",
    ),
    path(
        "youtube/connect/",
        YoutubeAuthViewSet.as_view({"post": "connect"}),
        name="youtube-auth-connect",
    ),
    # Facebook uses a ViewSet with auth-url, pages, and connect actions
    path(
        "facebook/auth-url/",
        FacebookAuthViewSet.as_view({"get": "auth_url"}),
        name="facebook-auth-url",
    ),
    path(
        "facebook/pages/",
        FacebookAuthViewSet.as_view({"post": "pages"}),
        name="facebook-pages",
    ),
    path(
        "facebook/connect/",
        FacebookAuthViewSet.as_view({"post": "connect"}),
        name="facebook-auth-connect",
    ),
    # Instagram uses a ViewSet with auth-url and connect actions (same pattern as Facebook/YouTube)
    path(
        "instagram/auth-url/",
        InstagramAuthViewSet.as_view({"get": "auth_url"}),
        name="instagram-auth-url",
    ),
    path(
        "instagram/connect/",
        InstagramAuthViewSet.as_view({"post": "connect"}),
        name="instagram-auth-connect",
    ),
    # TikTok uses a ViewSet with auth-url and connect actions (same pattern as Facebook/YouTube/Instagram)
    path(
        "tiktok/auth-url/",
        TiktokAuthViewSet.as_view({"get": "auth_url"}),
        name="tiktok-auth-url",
    ),
    path(
        "tiktok/connect/",
    ...
    path(
        "linkedin/auth-url/",
        LinkedinAuthViewSet.as_view({"get": "auth_url"}),
